...
class PE(PokerEval):
    ...
    def showdown_equities(cls, engine):
        ...
        for hrp in product(*hand_ranges):
            ...
            if len(hrp) == 2:
                ...
                # logger.debug('cards {} on board {}'.format(hrps, engine.board))
                eval = cls.pokereval_2(*engine.board, *hrps)
            else:
                logger.warn('exiting soon for large hrp')
                return {s: 1 / seats_len for s in seats}
            # elif len(hrp) == 3:
            #     hrps = list(chain.from_iterable(map(list, hrp)))
            #     logger.debug('cards {} on board {}'.format(hrps, engine.board))
                # eval = cls.pokereval_3(*engine.board, *hrps)
            # else:
            #     hrps = list(map(list, hrp))
            #     logger.debug('cards {} on board {}'.format(hrps, engine.board))
                # eval = cls.pokereval_n(engine.board, hrps)
        ...
        for s in seats:
            ...
            # logger.debug('player_hand_probs {}'.format(player_hand_probs))
            i = 0
        ...
        logger.info('cache info 2: {}'.format(cls.pokereval_2.cache_info()))
        logger.info('cache info 3: {}'.format(cls.pokereval_3.cache_info()))
        return equities
    ...
    def pokereval_3(cls, b1, b2, b3, b4, b5, c1, c2, c3, c4, c5, c6):
        board = [b1, b2, b3, b5, b5]
        hrp = [[c1, c2], [c3, c4], [c5, c6]]
        # logger.debug('reconstructed b= {} & c= {}'.format(board, hrp))
        eval = cls.poker_eval(cls, board=board, pockets=hrp,
                              iterations=cls.ITERATIONS_FAST, game=cls.GAME)
        # logger.debug('{} => {}'.format(hrp, [e['ev'] for e in eval['eval']]))
...
class MonteCarlo:
    ...
    def add_actions(self, e, parent):
        ...
        for a in actions:
            ...
            if a != 'bet':
                ...
                if e.q[0][0] == self.hero:
                    bets_percs = ACTIONS_HERO
                else:
                    bets_percs = ACTIONS_FOES
        ...
        # if bet calculate amount
        # if bet then no bets before, so only have to proportion it to pot
        if 'bet' in actions:
        ...
        if 'allin' in actions and e.phase != e.PHASE_RIVER:
