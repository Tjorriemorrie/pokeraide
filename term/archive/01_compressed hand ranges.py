...
class PE(PokerEval):
    ...
    #     Exclude dead cards
    #
    #     Only players that are 'in' are returned. There retain their seat
    #       position for the return.
    #
    #     To save cpu cycles only the hero's hand will be summed.
    #     Villains' hands
    #
    #         - extract hand ranges for players that are still 'in'
    #             * due to sampling these hand ranges must be shuffled
    #             * easier to shuffle here than to shuffle final combinations list as
    #                 the list is too big (current sample size is ~5%)
    #
    #         - need to be aggregated per pocket
    #             * since there are multiple combinations for each pocket
    #
    #         - calculate equities
    #             * every seat will have list of their equities
    #             * then sort the equities list to place higher win cards first
    #
    #         - calculate hand strength
    #             * from betting behaviour (accumulated during phases)
    #             * the hand strenghts are normalized here for each player
    #
    #         - weigh calculated equities with strength
    #
    #         - normalize all player's final equity (to 1)
    #     """
    #     logger.info('calculating winners for hands with {} samples'.format(cls.SAMPLE_SIZE))
    #
    #     hero_pocket = engine.data[hero]['hand']
    #     logger.debug('hero hand = {}'.format(hero_pocket))
    #
    #     seats = []
    #     hand_ranges = []
    #     for s, d in engine.data.items():
    #         if 'in' in d['status']:
    #             logger.debug('player {} is {}'.format(s, d['status']))
    #             seats.append(s)
    #             if s == hero_pocket:
    #                 hand_ranges.append(hero_pocket)
    #                 logger.debug('added hero pocket {} to hand_ranges'.format(hero_pocket))
    #             else:
    #                 hr = engine.players[s]['hand_range']
    #                 random.shuffle(hr)
    #                 hand_ranges.append(hr)
    #                 logger.debug('added {} hand range pockets for {}'.format(len(hr), s))
    #
    #     board_len = len(Counter(engine.board))
    #     n = len(hand_ranges)
    #     if n < 2:
    #         raise ValueError('Not enough pockets given to calculate winners')
    #
    #     sample_size = cls.SAMPLE_SIZE / n
    #     # hand_ranges = [random.sample(hr, min(len(hr), sample_size)) for hr in hand_ranges]
    #     hand_ranges = [hr[:ceil(len(hr) * sample_size)] for hr in hand_ranges]
    #     for _, hr in enumerate(hand_ranges):
    #         logger.debug('using {} hands for seat {}'.format(len(hr), _))
    #     cls.SAMPLE_SIZE *= 1.01
    #
    #     logger.info('calculating equities for {} players with {}'.format(n, engine.board))
    #     equities = {s: {} for s in seats}
    #     pps = list(product(*hand_ranges))
    #     logger.debug('{} product combinations of hand ranges samples'.format(len(pps)))
    #
    #     # random.shuffle(pps)
    ...
    #     for pp in pps:
    #         calcs += 1
    #         # logger.debug('cards {} on board {}'.format(pp, engine.board))
    #
    #         cnts = Counter([c for h in list(pp) + engine.board for c in h])
    #         # logger.debug('pp counted = {}'.format(cnts.most_common()))
    #         if len(cnts) < n * 2 + board_len:
    #             # logger.warn('duplicated cards found {} < {} * 2 + {}'.format(len(cnts), n, board_len))
    #             continue
    #
    #         eval = cls.poker_eval(cls, board=engine.board, pockets=list(map(list, pp)),
    #                               iterations=cls.ITERATIONS_FAST, game=cls.GAME)
    #         # logger.info('{} => {}'.format(pp, [e['ev'] for e in eval['eval']]))
    #
    #         for s, r, p in zip(seats, eval['eval'], pp):
    #             try:
    #                 equities[s][p].append(r['ev'] / 1000)
    #             except KeyError:
    #                 equities[s][p] = [r['ev'] / 1000]
    #             # logger.debug('player {} with hand {} = {}'.format(s, p, equities[s][p]))
    #
    #         if time.time() - time_start >= 0.50:
    #             cls.SAMPLE_SIZE *= 0.98
    #             logger.warn('exiting after taking too long. sample size now = {}'.format(cls.SAMPLE_SIZE))
    #             break
    #
    #     # logger.info('{}% completed [{}/{}] after {}s'.format(
    #     #     int(calcs / len(pps) * 100), calcs, len(pps), time.time() - time_start))
    #     # logger.info('sample size = {}'.format(cls.SAMPLE_SIZE))
    #
    #     # logger.debug('sorting hand range equities')
    #     equities_ranked = {s: {} for s in seats}
    #     for s, player_equities in equities.items():
    #         equity_ranked = SortedDict(neg, {(sum(e) / len(e)) + (random.random() / 10 ** 8): p
    #                                          for p, e in player_equities.items()})
    #         # logger.debug(equity_ranked)
    #         equities_ranked[s] = equity_ranked
    #     # logger.debug('hand range equities sorted')
    #
    #     hand_probs = {s: [1] if s == hero_pocket else engine.player_hand_strength(s) for s in seats}
    #
    #     # logger.info('calculating weighed equities')
    #     weighed_equities = {s: [] for s in seats}
    #     for s in seats:
    ...
    #             i += 1
    #             player_hand_prob_perc = player_hand_probs[int(i / len(player_equities_ranked))]
    #             player_hand_equity = player_hand_prob_perc * pocket_equity
    #             # logger.debug('player_hand_equity {} = {} * {}'.format(
    #             #     player_hand_equity, player_hand_prob_perc, pocket_equity))
    #             weighed_equities[s].append(player_hand_equity)
    #     # logger.info('weighed equities calculated')
    #
    #     equities = {s: sum(e) / len(e) for s, e in weighed_equities.items()}
    #     # logger.info('sum equities: {}'.format(equities))
    #
    #     total_equities = sum([e for e in equities.values()])
    #     equities = {s: e / total_equities for s, e in equities.items()}
    #     logger.error('equities normalized: {}'.format(equities))
    #     return equities

    @classmethod
    def compress_hand_ranges(cls, players, data):
        """From the total card combinations for each player, compress """
        logger.info('compressing hand ranges')
        hand_ranges = {}
        for s, d in data.items():
            if 'in' in d['status']:
                logger.debug('player {} is {}'.format(s, d['status']))
                hand_ranges[s] = []
                if '__' not in data[s]['hand']:
                    specific_pocket = tuple(data[s]['hand'])
                    hand_ranges[s].append(specific_pocket)
                    logger.debug('added specific pocket {} to hand_ranges'.format(specific_pocket))
                else:
                    cache = []
                    hr_unique = []
                    hr = players[s]['hand_range']
                    for hr_item in hr:
                        abstract = pockets_abstracts[hr_item]
                        if abstract in cache:
                            # logger.debug('hand {} abstract {} in cache, skipping'.format(hr_item, abstract))
                            pass
                        else:
                            hr_unique.append(hr_item)
                            cache.append(abstract)
                            # logger.debug('added {} to hand range and {}'.format(hr_item, abstract))
                    hand_ranges[s] = hr_unique
                    logger.debug('added {} hand range pockets for {}'.format(len(hr_unique), s))

        if len(hand_ranges) < 2:
            raise ValueError('Not enough pockets given to calculate winners')
        return hand_ranges
    
    @classmethod
    def calculate_hand_equities(cls, hand_ranges, board):
        seats_len = len(hand_ranges)
        board_len = len(Counter(board))
        # logger.debug('calculating equities for {} players with {}'.format(seats_len, board))

        pps = list(product(*hand_ranges.values()))
        # logger.debug('{} product combinations of hand ranges samples'.format(len(pps)))

        time_start = time.time()
        equities = {s: {} for s in hand_ranges}
        for _, pp in enumerate(pps):
            cnts = Counter([c for h in list(pp) + board for c in h])
            # logger.debug('pp counted = {}'.format(cnts.most_common()))
            if len(cnts) < seats_len * 2 + board_len:
                # logger.debug('duplicated cards found {} < {} * 2 + {}'.format(len(cnts), seats_len, board_len))
                continue

            eval = cls.poker_eval(cls, board=board, pockets=list(map(list, pp)),
                                  iterations=cls.ITERATIONS_FAST, game=cls.GAME)
            # logger.info('#{} for {} => {}'.format(_, pp, [e['ev'] for e in eval['eval']]))

            for s, r, p in zip(hand_ranges, eval['eval'], pp):
                try:
                    equities[s][p].append(r['ev'] / 1000)
                except KeyError:
                    equities[s][p] = [r['ev'] / 1000]
                # logger.debug('player {} with hand {} = {}'.format(s, p, equities[s][p]))
        logger.debug('{} combinations took {}s'.format(len(pps), time.time() - time_start))
        return equities

    @classmethod
    def weigh_equities_with_probs(cls, players, compressed_hand_ranges, equities_ranked, hand_probs):
        """Weigh the equity for the hand with the probability of being played. This needs
        to be done for every pocket, and the equity used from the abstract eval. To do
        that the hand range needs to be looped, the eval from the equities_ranked to be
        ...
        for s in compressed_hand_ranges:
            actual_hand_probs = []
            actual_hand_evals = []

            player_compressed_hand_range = compressed_hand_ranges[s]
            # logger.debug('player compressed hand range {} {}'.format(len(player_compressed_hand_range),
            #                                                          player_compressed_hand_range))
            player_full_hand_range = players[s]['hand_range']
            # logger.debug('player full hand range {} {}'.format(len(player_full_hand_range),
            #                                                          player_full_hand_range))
            player_equities_ranked = equities_ranked[s]
            # logger.debug('player_equities_ranked length = {}'.format(len(player_equities_ranked)))

            player_hand_probs = hand_probs[s]
            # logger.debug('player_hand_probs length = {}'.format(len(player_hand_probs)))

            i = 0
            # todo change this to loop over full hand range (uncompressed)
            for pocket_equity, pocket_cards in player_equities_ranked.items():
                prob_i = int(i / len(player_equities_ranked))
                player_hand_prob_perc = player_hand_probs[prob_i]
                i += 1

                # with pocket_cards sample used fetch its abstract value
                # abstract_pockets is in specific combinations (thus the way we entered it
                # might need to be the other way around)
                try:
                    pocket_cards_abstract = pockets_abstracts[pocket_cards]
                except KeyError:
                    pocket_cards_abstract = pockets_abstracts[pocket_cards[::-1]]
                # logger.debug('pocket_card {} of player is abstractly {}'.format(pocket_cards_abstract, pocket_cards))
                # with abstract, fetch list of cards from abstract
                list_abstract_cards = [apc for apc, apa in pockets_abstracts.items() if apa == pocket_cards_abstract]
                # logger.debug('abstract {} gives list {}'.format(pocket_cards_abstract, list_abstract_cards))
                # filter hand range for list of cards
                list_pocket_cards = [hpc for hpc in player_full_hand_range if hpc in list_abstract_cards]
                # logger.debug('{} cards found (from abstract list): {}'.format(len(list_pocket_cards), list_pocket_cards))

                for ppc in list_pocket_cards:
                    actual_hand_probs.append(player_hand_prob_perc)
                    actual_hand_evals.append(pocket_equity)
                # logger.debug('player {} has {}x pocket {} has {} equity with {} prob '.format(
                #     s, len(list_pocket_cards), pocket_cards_abstract, pocket_equity, player_hand_prob_perc))

            # actual_hand_probs_sum = sum(actual_hand_probs)
            equity = sum([he * hp for he, hp in zip(actual_hand_evals, actual_hand_probs)])
            weighed_equities[s] = equity
            # logger.info('player {} has {} equity'.format(s, equity))

        return weighed_equities

    @classmethod
    def showdown_equities(cls, engine, hero):
        """Calculate winning equities for the players given on engine. The hero will have only one hand
        whilst the villians' hand ranges will be dumbed down to the starting hands groups.
        ...
        Villains' hands
            ...
            - normalize all player's final equity (to 1)
        """
        # logger.info('calculating players equities for hands')

        compressed_hand_ranges = cls.compress_hand_ranges(engine.players, engine.data)
        
        hand_equities = cls.calculate_hand_equities(compressed_hand_ranges, engine.board)
        
        # logger.debug('sorting hand range equities')
        equities_ranked = {s: {} for s in compressed_hand_ranges}
        # logger.debug('hand_equities pre-sorting: {}'.format(hand_equities))
        for s, player_equities in hand_equities.items():
            equity_ranked = SortedDict(neg, {(sum(e) / len(e)) + (random.random() / 10 ** 8): p
                                             for p, e in player_equities.items()})
            equities_ranked[s] = equity_ranked
            # logger.debug('player {} with equity ranked {}'.format(s, equity_ranked))
        # logger.debug('hand range equities sorted')

        hand_probs = {s: [1] if len(compressed_hand_ranges[s]) == 1 else engine.player_hand_strength(s)
                      for s in compressed_hand_ranges}
        # logger.debug('hand probabilities from strengths normalized')
        # logger.debug(hand_probs)

        weighed_equities = cls.weigh_equities_with_probs(engine.players, compressed_hand_ranges,
                                                         equities_ranked, hand_probs)

        total_equities = sum([e for e in weighed_equities.values()])
        equities = {s: e / total_equities for s, e in weighed_equities.items()}
        logger.info('equities normalized: {}'.format(equities))
        return equities
    ...
    def process_node(self, e, n):
        ...
        if n.is_leaf():
            ...
            # percentage of hero as amt
                ...
                equities = PE.showdown_equities(e, self.hero)
