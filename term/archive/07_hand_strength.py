...
class ES:
    ...
    def player_stats(cls, engine, seat):
        ...
        for phase in ['preflop', 'flop', 'turn', 'river']:
            if not d[phase]:
                logger.debug('player does not have data for phase {}'.format(phase))
                break
            # add previous action (and
            phase_matching.append({'match': {'{}_1'.format(phase): {'query': d[phase][0].lower(), 'boost': 5}}})
            if d[phase][0] == d[phase][0].upper():
                phase_matching.append({'match': {'{}_aggro'.format(phase): {'query': True, 'boost': 8}}})
            if len(d[phase]) > 1:
                phase_matching.append({'match': {'{}_2'.format(phase): {'query': d[phase][1].lower(), 'boost': 5}}})
        ...
        max_contrib = max(pd['contrib'] for pd in engine.data.values())
        contrib_short = max_contrib - d['contrib']
        if contrib_short:
            is_limp = True if engine.phase == engine.PHASE_PREFLOP and max_contrib == engine.bb_amt else False
            if not is_limp:
                ...
                logger.info('facing aggro: no, is limping {}'.format(is_limp))
        else:
    ...
    def dist_player_stats(cls, stats, strength=False):
        ...
        for _ in range(20):
            ...
            k = dist.iloc[i_pos]
            v = dist[k]
            r += v.upper() if strength[0] <= p <= strength[1] else v.lower()
...
class Engine:
    ...
    def __repr__(self):
        ...
        for s, p in self.players.items():
            ...
            if self.phase != self.PHASE_SHOWDOWN:
                ...
                dist_stats = ES.dist_player_stats(d['stats'], d['strength'])
                r += '{:>63}\n\n'.format(dist_stats)
        ...
        # can act
            ...
            if d['status'] != 'allin':
                ...
                if self.phase == self.PHASE_PREFLOP and max(contribs) == self.bb_amt:
                    ...
                    # technically labels 'raise' but special handling for betting at add_actions
                    actions.append('bet')
    ...
    def do(self, action):
        ...
        if self.mc and action[0] not in ['labels', 'b', 'f', 'c', 'r', 'gg']:
        ...
        if action[0] == 'f':
            d['status'] = 'fold'
            d[self.phase] += ['f'.upper() if contrib_short else 'f']
            d['hand'] = ['  ', '  ']
        ...
        if action[0] in ['b', 'r']:
            ...
            if not int(action[1]) - contrib_short and 'l' not in d[self.phase]:
            ...
            if int(action[1]) >= p['balance'] - d['contrib']:
                ...
                d[self.phase] += [action[0].upper() if contrib_short else action[0]]
        ...
        if action[0] in ['k', 'c']:
            ...
            if d['contrib'] >= p['balance']:
                ...
                is_limp = True if self.phase == self.PHASE_PREFLOP and max_contrib == self.bb_amt else False
                d[self.phase] += [action[0].upper() if contrib_short and not is_limp else action[0]]
        ...
        if action[0] == 'labels':
            ...
            d[self.phase] += ['labels'.upper() if contrib_short else 'labels']
            d['status'] = 'allin'
            d['contrib'] = p['balance']
    ...
    def adjust_strength(self, s, d, a):
        ...
        # logger.info('adjusting strength for action {}'.format(labels))
        ...
        # update
        # adjust stats with next round values
        # fetch dist to generate min and max boundaries of action
        d['stats'] = ES.player_stats(self, s)
        dist = ES.dist_player_stats(d['stats'])
        a_range = [k for k, v in dist.items() if v == a]
        if not a_range:
            logger.warn('action {} not found in dist {}'.format(a, d['stats']))
            return

        logger.info('action {} has min {} and max {} from range {}'.format(a, min(a_range), max(a_range), a_range))

        old_strength = d['strength']
        new_strength = (
            (old_strength[0] + min(a_range)) / 2,
            (old_strength[1] + max(a_range)) / 2
        )
        d['strength'] = new_strength
        logger.debug('old strength {} to new strength {}'.format(old_strength, new_strength))
...
class PE(PokerEval):
    ...
    def showdown_equities_2(cls, engine, seats, hand_ranges, seats_len):
        ...
        # have now labels list of equities for every player
        ...
        hss = {s: (0, 1) if len(hr) == 1 else engine.data[s]['strength']
...
class MonteCarlo:
    ...
    def add_actions(self, e, parent):
        ...
        if 'raise' in actions and any(pa in d[e.phase] for pa in 'br'):
        ...
        go_allin = stats.get('allin', 0.01) if e.phase == e.PHASE_RIVER else 0
        ...
        non_fold_equity = 1 - stats.get('fold', 0)
...
class Game:
    ...
    def save_game(self):
        ...
        for s, d in self.engine.data.items():
            ...
            for phase in ['preflop', 'flop', 'turn', 'river']:
                if d[phase]:
                    doc['{}_aggro'.format(phase)] = True if d[phase][0] == d[phase][0].upper() else False
                    doc['{}_1'.format(phase)] = d[phase][0].lower()
                if len(d[phase]) > 1:
                    doc['{}_2'.format(phase)] = d[phase][1].lower()
            if doc:
                doc.update({
...
@lru_cache()
def cached(a, b, c):
    logger.info('labels={} b={} c={}'.format(a, b, c))
    if a == 1:
        return 'labels'
    if b == 1:
        return 'b'
