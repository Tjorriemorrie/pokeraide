...
class Engine:
    ...
    def player_hand_strength(self, s):
        ...
        Convert the hand strength to labels multiplier
        Give the multiplier labels small effect"""
        ...
        deviation = 0.05
        limit_bot = 1 - deviation
        limit_top = 1 + deviation
        limit_rng = limit_top - limit_bot
        logger.debug('scaling between {} and {} (dev of {})'.format(limit_bot, limit_top, deviation))

        hs_multis = [(x * limit_rng) + limit_bot for x in hs_scaled]
    ...
    def create_hand_strength(self, p, d, bet_to_pot):
        ...
        for d in range(1, 100):
            i_fwd = iloc + d
            if i_fwd <= 100:
                hs_row[i_fwd] = round(1 - d * 0.01, 2)
            i_bwd = iloc - d
            if i_bwd >= 0:
                hs_row[i_bwd] = round(1 - d * 0.01, 2)
...
class PE(PokerEval):
    ...
    def showdown_equities_2(cls, engine, seats, hand_ranges, seats_len):
        ...
        # have now labels list of equities for all the hand combinations
        ...
        hs_multis = {s: [1] if len(hs) == 1 else engine.player_hand_strength(s)
        ...
        for s in seats:
            ...
            logger.debug('player_hs_multis {}'.format(player_hs_multis))
            i = 0
            for pocket_equity, pocket_cards in player_equities_ranked.items():
                player_hs_multi = player_hs_multis[i // player_equities_ranked_len]
                player_hand_equity = pocket_equity * player_hs_multi
                logger.debug('player_hand_equity i{}: {} = {} * {}'.format(
                    i, player_hand_equity, pocket_equity, player_hs_multi))
                equities_weighed[s].append(player_hand_equity)
                i += 1

        # now have the equities weighed by the hand strengths

        # todo: remove this as the hand counts are equal, the sum is fine, just scale it to 1
        # equities_agg = {s: sum(e) / len(e) for s, e in equities_weighed.items()}
        # equities_agg = {s: np.mean(e) for s, e in equities_weighed.items()}
        # logger.debug('player equities averaged: {}'.format(equities_agg))

        # final normalization to return p~1
        total_equities = sum(sum(e) for e in equities_weighed.values())
        equities = {s: sum(e) / total_equities for s, e in equities_weighed.items()}
        ...
        while not self.is_complete and time.time() - time_start < timeout:
            ...
            for leave in leaves:
                ...
                if time.time() - time_start >= timeout:
                    break
        duration = time.time() - time_start
        logger.warn('Monte Carlo {}/s ended after {} taking {}s'.format(i // duration, i, int(duration)))
    ...
    def load_player_stats(self, players):
        ...
        for s, p in players.items():
            ...
            for phase in ['preflop', 'flop', 'turn', 'river']:
                ...
                # make labels dict out of the actions
